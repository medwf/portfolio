        ...
        <nav className="hidden lg:flex items-center gap-6">
          {navItems.map((item) => (
            <button
              ...
              className={`text-sm font-medium transition-colors relative group ${activeSection === item.href.substring(1)
                ? "text-primary"
            ...
            <nav className="container px-4 sm:px-6 py-4 sm:py-6 flex flex-col gap-3 sm:gap-4">
              {navItems.map((item, index) => (
                ...
                >
                  <button
                    onClick={() => scrollToSection(item.href)}
                    className={`text-lg font-medium transition-colors py-2 block w-full text-left ${activeSection === item.href.substring(1)
                      ? "text-primary"
                      : "text-muted-foreground hover:text-foreground"
                      }`}
                  >
